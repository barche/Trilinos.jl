...
cmake_prefix = (TRILINOS_ROOT == "" ? "" : "$TRILINOS_ROOT;")*jlcxx_dir
...
trilinos_steps = @build_steps begin
	`cmake -G "$genopt" -DCMAKE_INSTALL_PREFIX="$prefix" -DCMAKE_BUILD_TYPE="$build_type" -DCMAKE_PREFIX_PATH="$cmake_prefix" -DCMAKE_PROGRAM_PATH="$JULIA_HOME" -DCMAKE_CXX_COMPILER=mpic++ -DCMAKE_C_COMPILER=mpicc $trilinoswrap_srcdir`
